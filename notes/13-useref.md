...
[^1]:
    **useCallback**:
    ...
    hook is inside a fiber &rarr; `Hook {memoizedState: the function,deps: [a, b],next: Hook | null}` &rarr; on next render &rarr; react compares depnd. &rarr; if not changed &rarr; reuse the existing function &rarr; `const handleClick = previousHook.memoizedState;` &rarr; if changed &rarr; create a new function.
